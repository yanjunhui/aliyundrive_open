...
type FileOption struct {
	...
	Name                string               `json:"name,omitempty"`                  // 文件名(重命名必填)
	Path                string               `json:"path,omitempty"`                  // 文件完整路径(不包括 /root)
	ExpireSec           int64                `json:"expire_sec,omitempty"`            // 下载链接有效期(链接必填)
	URLExpireSec        int64                `json:"url_expire_sec,omitempty"`        // 视频播放地址有效期(播放必填)
	ToParentFileID      string               `json:"to_parent_file_id,omitempty"`     // 移动到的目录ID(移动必填)
	CheckNameMode       CheckNameMode        `json:"check_name_mode,omitempty"`       // 重命名时检查文件名模式(重命名)
	NewName             string               `json:"new_name,omitempty"`              // 移动时重名时的新文件名(移动)
	Marker              string               `json:"marker"`                          // 分页标记(目录)
	Limit               int64                `json:"limit,omitempty"`                 // 分页大小(目录)
	OrderBy             OrderSortedField     `json:"order_by,omitempty"`              // 排序字段(目录)
	OrderDirection      OrderSortedDirection `json:"order_direction,omitempty"`       // 排序方式(目录)
	Category            string               `json:"category,omitempty"`              // 指定返回的文件类型(目录/文件)
	Type                FileType             `json:"type,omitempty"`                  // 指定返回文件还是目录, type不为空时, category参数无效(目录)
	VideoThumbnailTime  int64                `json:"video_thumbnail_time,omitempty"`  // 视频预览时间 (单位:秒) (目录/文件)
	VideoThumbnailWidth int64                `json:"video_thumbnail_width,omitempty"` // 视频预览宽度 (目录/文件)
	ImageThumbnailWidth int64                `json:"image_thumbnail_width,omitempty"` // 视频预览图片宽度 (目录/文件)
	Fields              string               `json:"fields,omitempty"`                // 只返回指定字段 (目录)
	PartInfoList        []FileUpdatePartInfo `json:"part_info_list"`                  // 分片上传信息(上传)
	OpenFile            *os.File             `json:"-"`                               // 文件流(上传)
	UploadID            string               `json:"upload_id,omitempty"`             // 上传ID(上传)
}
